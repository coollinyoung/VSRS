...
全部檔案複製成功後，自動執行目的磁區的 `os\隱藏資料夾.bat`，並以該 `os` 資料夾為工作目錄。程式會等待批次檔結束並記錄結果；若複製失敗則不執行批次檔。批次檔不存在或執行失敗時，會明確提示「檔案已複製完成，但後續作業失敗」。

- 按「自動搜尋隨身碟」，程式會尋找各磁碟根目錄下名為 `ventoyhdd` 的資料夾，也可按「瀏覽」手動選取。
- 目的磁區列出所有已偵測到的磁區，不再依 USB、Windows 資料夾或實體磁碟編號篩選；仍優先選取標籤為 `Ventoy` 的磁區。執行前請核對代號、容量與標籤。
